...
impl AboutWindow {
    ...
    pub fn new(parent: &impl IsA<gtk::Window>, app_name: &str, app_id: &str, version: &str, icon: &str, translate_url: &str, issue_url: &str, more_info_url: &str, translators: &[&str], developers: &[&str], copyright_year: i32, license: AboutWindowLicenses, color: Colors) -> AboutWindow {
        let translators_length1 = translators.len() as i32;
        let developers_length1 = developers.len() as i32;
        unsafe {
...
impl Default for AboutWindow {
                     fn default() -> Self {
                         glib::object::Object::new::<Self>(&[])
                            .expect("Can't construct AboutWindow object with default parameters")
                     }
                 }
...
impl<O: IsA<AboutWindow>> AboutWindowExt for O {
    ...
    fn translator_names(&self) -> Vec<glib::GString> {
        unsafe {
            ...
            let ret = FromGlibContainer::from_glib_none_num(ffi::he_about_window_get_translator_names(self.as_ref().to_glib_none().0, result_length1.as_mut_ptr()), result_length1.assume_init() as usize);
            ret
        }
    }

    fn set_translator_names(&self, value: &[&str]) {
        let value_length1 = value.len() as i32;
        unsafe {
    ...
    fn developer_names(&self) -> Vec<glib::GString> {
        unsafe {
            let mut result_length1 = mem::MaybeUninit::uninit();
            let ret = FromGlibContainer::from_glib_none_num(ffi::he_about_window_get_developer_names(self.as_ref().to_glib_none().0, result_length1.as_mut_ptr()), result_length1.assume_init() as usize);
            ret
        }
    }
